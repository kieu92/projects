...
class Cylinder : public Object{
  public:
    // Code is based on NYU Cylinder-Ray Intersections
    Cylinder(const Vec3f &a, const Vec3f &b, const float &r) : pa(a), pb(b), radius(r), radius2(r * r) {}
    bool intersect(const Vec3f &orig, const Vec3f &dir, float &tnear, uint32_t &index, Vec2f &uv) const
    {
      Vec3f delta_p = orig - pa;
      Vec3f va = pb - pa;
      float a = dotProduct(dir - dotProduct(dir, va) * va, dir - dotProduct(dir, va) * va);
      float b = 2 * dotProduct(dir - dotProduct(dir, va) * va, delta_p - dotProduct(delta_p, va) * va);
      float c = dotProduct(delta_p - dotProduct(delta_p, va) * va, delta_p - dotProduct(delta_p, va) * va) - radius2;
      float t1, t2;
      if (!solveQuadratic(a, b, c, t1, t2)) return false;
      if (t1 < 0 && t2 < 0) return false;

      Vec3f q1 = orig + dir * t1;
      Vec3f q2 = orig + dir * t2;

      if (dotProduct(va, q1 - pa) > 0 && dotProduct(va, q1 - pb) < 0) {
        tnear = t1;
      }

      if (dotProduct(va, q2 - pa) > 0 && dotProduct(va, q2 - pb) < 0) {
        if (tnear < t2) {
          tnear = t2;
        }
      }
    ...
    { 
      if (P.y == pb.y) {
        ...
        // Body of Cylinder
        N = normalize(P - Vec3f(pa.x, P.y, pa.z)); 
      }
    }
...
public:
    Box(const Vec3f &a, const Vec3f &b) : vmin(a), vmax(b) {}
    bool intersect(const Vec3f &orig, const Vec3f &dir, float &tnear, uint32_t &index, Vec2f &uv) const
    {
      float tmin = (vmin.x - orig.x) / dir.x;
      float tmax = (vmax.x - orig.x) / dir.x;

      if (tmin > tmax) std::swap(tmin, tmax);

      float tymin = (vmin.y - orig.y) / dir.y;
      float tymax = (vmax.y - orig.y) / dir.y;
      ...
      float tzmin = (vmin.z - orig.z) / dir.z;
      float tzmax = (vmax.z - orig.z) / dir.z;
...
int main(int argc, char **argv)
{
    ...
    */
    ...
    Box *b1 = new Box(Vec3f(0.5, -0.5, -8), Vec3f(2.5, 1.5, -6));
    b1->materialType = DIFFUSE_AND_GLOSSY;
    b1->diffuseColor = Vec3f(1.0, 0.0, 0.0);

    Cylinder *c1 = new Cylinder(Vec3f(0.5, -1.5, -8), Vec3f(0.5, 1.5, -8), 1.0);
    c1->materialType = DIFFUSE_AND_GLOSSY;
    c1->diffuseColor = Vec3f(0.6, 0.7, 0.8);


    // objects.push_back(std::unique_ptr<Sphere>(sph1));
    // objects.push_back(std::unique_ptr<Sphere>(sph2));
    objects.push_back(std::unique_ptr<Box>(b1));
    objects.push_back(std::unique_ptr<Cylinder>(c1));
    ...
    lights.push_back(std::unique_ptr<Light>(new Light(Vec3f(0.5, 0, 0), 2)));
